...
**ページのクエリ文字列は既定で捨てます**（静的ファイルはパスでしか引けない）。例外は `distan_variant_keys` で宣言したキーだけで、これらは `Distan_Urls::fold_query_segment()` で 1 つのパスセグメント（既定 `key-value`、複数はキー順に `_` 連結・各キー値は `rawurlencode`）に畳み込み、`/slug/tab-a/index.html` として出力します。畳み込みは `url_to_output_path()`（キュー＝ディスク名）と `convert_single()`（リンク href）の両方で同じ経路を通るので、ディスク名は復号形・href はエンコード形で自動的に整合します（多バイトスラッグと同じ扱い）。アセットのクエリ（キャッシュバスター）は従来どおり href に残します。宣言キー以外は無視するので、`utm_*` 等で列挙が膨張することはありません。
...
**HTML コメント削除（`Distan_Cleaner::strip_comments`、任意）**。設定 `strip_comments` が有効なとき、プレーンな `<!-- -->` を納品 HTML から除去します。危険な正規表現手術なので約束事が3つあります。(1) **順序**: `apply_page_markers` の後・書き出し直前（generator の 312→314 の間）で適用する。バッファ段（レンダリング直後）に置くと `<!-- distan:... -->` を先に消してマーカーが無効化されるため。(2) **保護**: `<script>`/`<style>`/`<textarea>`/`<pre>` を先に退避してからコメントを消し、あとで戻す（中身の `<!--` を誤爆させない）。(3) **温存**: IE 条件付きコメント（`<!--[if ...]>`）は実マークアップを囲みうるので残す。加えて `preg_replace_callback` が null を返したら入力をそのまま返す（無言の空白ページ化を防ぐ）。実装時は Python＋合成 HTML で先に検証し、デリミタは `~`（`#` 事故回避）で。
...
### 対象の絞り込み（`Distan_Markdown::wants`）

`content.md` に含める範囲を、投稿タイプ・公開日・個別ページで限定できます。**関門は content.md 側だけ**に置く——generator の書き出し直前で `is_enabled() && wants( $item )` を判定し、`false` ならその item を content.md に足さないだけ。静的 HTML（`dist/`）の生成は常に全ページ実行され、絞り込みの影響を受けません。「サイトは丸ごと、AI 向けは記事だけ」を成立させる要はここです。

- 二段構え。**フィルタ未設定（投稿タイプ・公開日・個別ページのどれも無し）なら全ページ通過**——投稿・CPT・固定ページ・トップ・アーカイブすべて（1.7 以前と同じ full export）。ここが既定。(1) 投稿タイプ（`md_post_types`。チェックしたタイプだけ／空なら投稿タイプでは絞らない）＋公開日（`md_date_from`／`md_date_to`、両端含む・片側省略可、`get_post_time( 'Y-m-d', false, $id )` の暦日文字列で比較）。**絞り込みが有効なときだけ**、投稿タイプを持たないアイテム（トップ・アーカイブ・404）と固定ページが外れる。(2) 個別の固定ページ（`md_pages`＝ページ ID）は絞り込みに関係なく常に採用。
- **個別指定は絞り込みを無視して常に採用**。1 枚ずつ人が選んだものは「宣言」であり、`distan:` マーカーと同じ思想（判断を人に委ねる／推測で落とさない）。逆に投稿タイプを持たない item（トップ・タームアーカイブ・404）は、**絞り込みが有効なときだけ** content.md から外れる（フィルタ未設定なら全ページの一部として入る）。
- 賢く自動判別しようとしない。「開発用の noindex」と「意図的な noindex」を見分けないのと同じで、何を AI に読ませるかは設定（人の宣言）に委ねる。既定（全設定が空）は従来どおり全ページ。
- UI は設定画面に Alpine で用意。投稿タイプはチェックボックス、公開日は min/max の日付入力、固定ページは**テンプレート書き出しと同じ検索＋セレクト方式**——タイトル検索で `<select>` の候補（`x-for`、`x-model` は付けない）が絞られ、ブラウザが該当を自動選択、「追加」ボタンで `picks` に積み、選んだページはチップで表示・個別解除できる。選択自体はサーバ側 PHP のゲート（`wants()`）で、Alpine は操作面のみ。検索で候補から外れても選択が送信から漏れないよう、送信値は `picks` から常時 hidden input に出す（可視のチェックボックスやセレクトには name を付けない）。
- 管理画面のアセット（`distan-admin.css` / `distan-admin.js`）は `filemtime()` を `?ver` に使って enqueue する（`file_exists` で存在確認、無ければ `DISTAN_VERSION` にフォールバック）。ファイルを変えるたびにクエリが変わるので、`DISTAN_VERSION` を据え置いたまま CSS を直しても実機に確実に反映される（同一バージョン内での反復時に古い CSS がキャッシュされ続ける問題の対策。生成物＝`dist/` 側のキャッシュバスティングとは別レイヤーの話）。
