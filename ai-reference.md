...
このファイル一枚で、Distan の全体像（考え方・導入・使い方・設定・運用・拡張）が把握できます。用途は2つあります。

1. **マニュアル生成** — このファイルを AI に渡し、「Distan のユーザーマニュアルを書いて」と頼めば、下記の情報から手順書を作れます。読者は制作会社の担当者（WordPress は分かるが Distan は初めて）を想定してください。おすすめの構成は「導入 → 基本の使い方 → 設定 → 運用（更新・差分納品）→ トラブル対処 → 拡張（フィルタ）」の順です。専門用語には都度ひとこと補足を添えると親切です。
2. **実装理解** — AI エージェントが Distan の挙動・境界・拡張点を素早く把握するための参照。コードを編集する際の設計は `CLAUDE.md`、人間向けの詳しい使い方は `README.md`、管理画面のデザインは `DESIGN.md` を参照。
...
- **動作要件**: WordPress、PHP 8.0 以上、そして **ループバック HTTP が通ること**（サイトが自分自身に HTTP リクエストできること）が唯一の必須要件。SQL 直叩き・`exec()`・外部バイナリ・外部 API は不要。Local / MAMP / XAMPP / DDEV / Docker / 共用サーバーで動作し、オフラインでも使える。
- **インストール**: WordPress.org の管理画面から検索してインストールするか、zip をアップロードして有効化。有効化すると管理メニューに「Distan」が追加され、プラグイン一覧の行にも「設定」リンクが付く。

## 基本の使い方（フロー）

1. **環境を確認する** — 「環境」タブ →「確認する」。ループバック通信が通るかを確認する（唯一の必須条件）。
2. **書き出す** — 「生成」タブ →「静的HTMLを書き出す」。公開中の全ページを巡回して `dist` に書き出す。1回あたりのページ数（バッチ）は調整可。生成のたびに全ページを作り直す（増分スキップはしない）。
3. **受け取る・公開する** — 「ZIPをダウンロード」で納品するか、「デプロイ」ボタン（有効化時）で自分の公開処理につなぐ。更新時は「差分ZIP（変更分のみ）」で変わった分だけ渡せる。
- 出力先は `wp-content/uploads/distan/dist`（固定・変更不可）。

## 設定項目（「設定」タブ）

- **公開URL** — canonical と OGP に使う本番 URL。内部リンクはドキュメント相対で書き出すため、設置場所は選ばない。
- **ファイル名の形式** — `/about/index.html`（既定）か `/about.html`。
- **内部リンクの書き方** — ドキュメント相対（既定・ZIP を解凍してそのまま開ける）か、公開URLで絶対指定（サーバーのドキュメントルート配置向け）。
...
- **HTMLコメントを削除**（`strip_comments`・既定オフ）— 納品 HTML からプレーンな HTML コメント（`<!-- ... -->`）を除去。`Distan_Cleaner::strip_comments()` を **`apply_page_markers` の後・書き出し直前**（generator 312→314 の間）で適用するため、`distan:` マーカーは既に消費・除去済みで競合しない。`<script>`/`<style>`/`<textarea>`/`<pre>` の中身は退避して保護、IE 条件付きコメント（`<!--[if ...]>`）は実マークアップを囲みうるため残す。PCRE 失敗時は入力をそのまま返す（空白ページ化を防ぐフェイルセーフ）。clean_html とは独立。
- **Markdown を書き出す** — 全ページ本文を 1 ファイル `content.md` に（AI ツール向け）。既定オフ。
  ...
  - Markdown の絞り込み（`Distan_Markdown::wants`）: **何も指定しなければ全ページ**（投稿・CPT・固定ページ・トップ・アーカイブ）が対象＝1.7 以前と同じ full export。絞り込みは任意で、投稿タイプ・公開日・個別ページを設定したときだけ効く。静的サイト（dist/）の生成内容には影響しない content.md 側だけのゲートで、判定は generator の書き出し直前 `is_enabled() && wants($item)`。フィルタ設定時の挙動——(1) **投稿タイプ＋公開日**: `md_post_types`（チェックしたタイプだけ／空なら投稿タイプでは絞らない）と `md_date_from`／`md_date_to`（YYYY-MM-DD、両端含む・片側省略可、サイトTZの暦日比較）。**絞り込みが有効なときは**投稿タイプを持たないアイテム（トップ・タームアーカイブ・404）と固定ページが外れる（固定ページは下の個別指定で戻す）。(2) **個別の固定ページ**: `md_pages`（ページ ID）。UI はテンプレート書き出しと同じ検索＋セレクト＋「追加」でチップ選択（`x-model` なしで候補が絞られ該当を自動選択、送信値は `picks` から hidden 出力）。投稿タイプ・公開日の絞り込みに関係なく常に採用（＝人の明示宣言）
- **サイトマップを書き出す** — `sitemap.xml`。除外パターンを指定できる。既定オフ。
- **robots.txt を書き出す** — 最小構成（`Allow: /`）。既定オフ。
- **差分ZIP** — 生成画面に「差分ZIP（変更分のみ）」を表示（既定オン。検知自体は常に走る）。
- **テンプレート書き出し** — 生成画面に「テンプレート書き出し」を表示（既定オン）。
- **デプロイボタン** — 生成画面に「デプロイ」ボタンを表示（既定オフ）。

## 運用

- **初回納品** — 全体 ZIP をダウンロードして納品・アップロード。
- **更新** — WordPress で編集 → 再生成 → 「差分ZIP」（変更ファイル＋削除指示の `DELETE.txt`）を本番に上書き。全体を上げ直さない。
- **差分の基準** — 生成とデプロイが同じ環境なら既定 `db` のまま。別環境（ローカルや CI で生成 → 別サーバーへ納品）なら `distan_manifest_source` を `output` にして、差分の基準を成果物と一緒に運ぶ。
- **デプロイ自動化** — `distan_after_generate`（毎回発火）/ `distan_dispatch`（手動ボタン）フックに rsync・git push・Webhook 等を繋ぐ。Distan 自体はデプロイせず、認証情報も持たない。

## トラブル対処

- **生成できない／ループバック不可** — サーバーが自分自身へ HTTP アクセスできるかを環境チェックで確認。ローカルの hosts 設定、Basic 認証、ファイアウォールが原因になりやすい。
- **リンク切れが報告される** — テーマがカテゴリー／著者／日付アーカイブへリンクしているが、それらが未生成、が典型。`distan_taxonomies` で対象を足すか、リンク側を調整する。Distan は自動修正しない（報告のみ）。
- **開発ドメインが残る** — canonical / OGP / JSON-LD に開発環境の URL が残る場合は、「公開URL」を設定して再生成すれば本番ドメインに置換される。
- **日時がずれる** — 表示日時は WordPress 本体のタイムゾーン設定（設定 → 一般）に従う。UTC のままだと実地の時刻とずれる。
- **キャッシュプラグイン** — 生成環境には入れない（古いキャッシュ HTML を取得する恐れ）。
...
- **Markdown 書き出し（選択制）** — 全ページ本文を 1 ファイル `content.md` にまとめる。Gemini Notebook（旧NotebookLM）等の AI ツール向け。ページネーションは除外。URL は公開 URL に置換（開発用に `content.local.md` も選択可）。投稿タイプ・公開日（min〜max）で対象を絞り込め、固定ページはタイトル検索で個別に含められる（記事だけを AI に読ませる等）。絞り込みは content.md 側だけに効き、静的サイトの生成内容は変えない
...
- **取りこぼしの取り込み** — 列挙は DB を読むので、プラグインが動的登録する URL（フォーム完了画面・仮想ルート）を知らない。コア sitemap の照合（`sitemap_missing`）やリンク切れレポートで存在は見えるが、生成に足すには従来 `distan_sources` を手書きする必要があった。生成画面の「取りこぼしの取り込み」が、その判断を URL ごとに記録する（`Distan_Takeup`、オプション `distan_takeup` に include/ignore の 2 リスト）。候補（コア sitemap にあって未生成の URL）を「含める／今後表示しない／未決」から選ぶ。既定はオプトインで、選んだものだけが `Distan_Collector::collect()` で `distan_sources` と同じ経路（make_item → dedup、provenance 付き、origin=takeup）でキューに入る。判断は記憶され、毎回選び直さない。ignore は disjoint（include より優先）で、書き出したくない URL が混ざらない。コア sitemap にも載らない URL は自由入力欄で足せる（同一オリジンのみ許可）。「検知」は既存（`missing_from` / `audit_links` の broken）、この機能が足すのは「取り込みアクション」
