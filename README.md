...
名前は似ていますが、発想は逆です。**ヘッドレス CMS** は表示（頭）を切り離し、CMS 本体（胴体）を API として本番で動かし続けます。**ヘッドロス CMS**（Distan）はその逆で、WordPress の表示結果＝静的 HTML（頭）だけを本番に送り、CMS 本体（胴体）は本番から丸ごと外します。本番で動くものは何もありません。

| | ヘッドレス CMS | ヘッドロス CMS（Distan） |
|---|---|---|
| 本番で動くもの | CMS が API として稼働し続ける | 何も動かない（静的 HTML だけ） |
| 表示（頭） | 別途フロントエンドを構築する | WordPress の表示を納品用に書き出す |
| CMS 本体（胴体） | 本番で稼働 | 手元の工房に残す（本番には無い） |
...
サイトが更新され続ける限り、それを書き出す制作環境も生き続ける必要があります。Distan がフロントエンドのツールチェーンを持ち込まないのは、初回の学習コストのためではなく、数年後にその環境が動くためです。

Astro や EJS のようにビルドを前提とする構成は、3 年後に `npm install` が通る保証がありません。依存が消えていたり、Node のバージョンが合わなかったりすれば、その時点でサイトを更新できなくなります。WordPress のローカル環境はこの点で頑丈です。PHP とデータベースがあれば動き、しばらく放置しても立ち上がり、社内の誰でも触れます。

管理画面の UI には Alpine.js を使っていますが、ビルドは挟まず 1 ファイルとして同梱し、`<script>` で読み込むだけで動きます（`npm install` もバンドラも不要）。「フロントに動きを付けること」と「数年後も動く環境であること」は両立します。ツールチェーンを持ち込まないとは、フロントエンドを避けることではなく、依存解決とビルドの工程を持ち込まないということです。
...
## 不要なアセットをページから外す（マーカー）

自前でテンプレートを組むと、WordPress の既定スタイルやプラグインの JS が、使っていないのに出力に残ることがあります。ページのテンプレート HTML にマーカーを書いておくと、Distan がそのページの出力から該当アセットを外します。中身は推測せず、テンプレートに書かれた指定にだけ従います。

このマーカーは、**通常の「静的HTMLを書き出す」でも、テンプレート書き出しでも効きます**（設定「WordPress の痕跡を除く」のようなサイト全体設定ではなく、マーカーを書いたそのページにだけ効きます）。

- `<!-- distan:no-block-styles -->` — ブロックエディタの標準スタイルと theme.json 由来のスタイルを、このページの出力から外します。`wp-block-library` の `<link>` に加え、WordPress が個別に吐く `wp-block-*` / `global-styles` 系のインライン `<style>`（`wp-block-library-inline-css` やブロックごとの `wp-block-*-inline-css`、各プレースホルダ）も対象です。画像の `sizes=auto` 用ヘルパー（`wp-img-auto-sizes-*`）のインライン `<style>` も対象です。
...
外したアセットは、参照している `<link>` / `<script>` タグごとページから取り除かれます。マーカーのコメント自体も出力からは除去されます。

**通常生成では、アセットのファイル本体は削除しません。** CSS・JS は複数ページで共有されるため、マーカーを書いたページの出力からは参照（タグ）を外しますが、ファイルは他のページのために残します。そのページはそのアセットを読み込まなくなり、他のページには影響しません。テンプレート書き出し（1 ページだけの受け渡し）では、そのページが参照しないアセットはそもそも同梱しないので、結果として含まれません。
...
- ヘッダー・ナビゲーション・フッター・サイドバー・フォームは除き、`<main>`（無ければ `<article>` や本文コンテナ）の**本文だけ**を抽出します。ページごとに、見出し・元 URL・メタディスクリプションを添えて連結します（`<title>` を出さないテーマでは、本文の最初の見出しをページタイトルに使います）。
- 本文の構造はできるだけ Markdown として保ちます。見出し、箇条書き、順序リスト（番号付き）、リンク、**表（Markdown テーブル）**、コード（インライン／フェンス）、引用、強調（太字・斜体）、画像（alt 付き）を変換します。AI に読み込ませたときに構造が伝わることを狙った変換で、完璧な HTML→Markdown 往復変換ではありません。
...
`ai-reference.md` は、Distan の全体像（考え方・導入・使い方・設定・運用・トラブル対処・拡張）を 1 枚にまとめた AI 向けリファレンスです。**このファイルを AI に渡して「Distan のユーザーマニュアルを書いて」と頼めば、手順書を生成できます。** 制作会社の担当者向けの導入マニュアル、社内向けの運用手順、設定リファレンスなど、渡し方を変えれば用途に合わせて出力できます。

同じファイルは、AI エージェントに Distan の挙動や拡張点（フィルタ）を理解させる用途にも使えます。
