...
管理画面の UI には Alpine.js を使っていますが、ビルドは挟まず 1 ファイルとして同梱し、`<script>` で読み込むだけで動きます（`npm install` もバンドラも不要）。「フロントに動きを付けること」と「数年後も動く環境であること」は両立します。ツールチェーンを持ち込まないとは、フロントエンドを避けることではなく、**依存解決とビルドの工程を持ち込まない**ということです。
